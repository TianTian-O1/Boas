...
void MLIRGen::initializeContext() {
    ...
    context->loadDialect<mlir::cf::ControlFlowDialect>();  // 添加控制流dialect
    // context->loadDialect<mlir::boas::BoasDialect>(); // 暂时注释掉
...
mlir::Value MLIRGen::generateMLIRForNode(const ExprAST* node) {
    ...
    try {
        ...
        switch (node->getKind()) {
            ...
            case ExprAST::Subscript: {
                // For now, treat subscript as a variable access
                // This is a simplified implementation that may need enhancement
                std::cerr << "[DEBUG] Handling Subscript node (simplified)\n";
                // Return a dummy value for now to avoid crashes
